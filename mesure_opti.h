...
void create_brut_data_opti(){
  FILE *P_FICHIER = NULL;
  char NOM_FICHIER[30] = "aa_brut_opti.dat";

  if(fopen(NOM_FICHIER, "r") == NULL){
    char title[1000] = "#t1 \t\t t2 \t\t t1-t2\n";
...
void fin_mesure_opti(double t1, clock_t a,double t2, clock_t b,int rank,
  ...
  if(rank ==0){
    ...
    if(nb_run == N-1){
      ...
      sprintf(VAL2, "%f", moyenne2(N,temps_clock)); //moyenne
      fprintf(P_FICHIER2," %s \t ",VAL2);
      sprintf(VAL2, "%f", b-a-ecart_type2(N,temps_clock)); // moyenne - ecart-type
      fprintf(P_FICHIER2,"%s \t ",VAL2);
      sprintf(VAL2, "%f", b-a+ecart_type2(N,temps_clock)); // moyenne + ecart-type
